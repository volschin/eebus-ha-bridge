...
> **Status: tooling not on `main`.** The capture implementation
> (`internal/eebus/extendedcapture.go`, its tests, and the `experimental.extended_capture`
> config knobs) was never merged. It lives only on the deleted branch
> `feat/vr940-diagnostics-usecases`, tip `ac5905d` — restore with
> `git branch feat/vr940-diagnostics-usecases ac5905d`.
>
> The branch does not apply to current `main`: it patches a `main()` that has since
> been split into `cmd/eebus-bridge/app.go`, and hooks in alongside the HVAC probe,
> which has been removed. Re-running a capture means reworking the integration.
>
> The captured evidence is kept here regardless, because it is the record of what
> this hardware does and does not publish — see
> `docs/superpowers/specs/2026-07-13-vr940-extended-capture-findings.md`. The rest of
> this document describes how the capture was produced.
