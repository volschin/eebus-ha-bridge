...
**Branch:** `feat/vr940-diagnostics-usecases` (deleted; tip `ac5905d`). The capture
tooling itself was never merged — see the status note in `docs/vr940-extended-capture.md`.

**Supersedes the open questions in** `docs/superpowers/specs/2026-07-13-vr940-diagnostics-and-extended-data-design.md`

**Phase 2 status, as of 2026-07-31:**

- Item 1, device operating state — **shipped** (PR #102, v0.10.4).
- Item 2, heating-zone label — **still open**; `userLabel` has no reader in the bridge
  or the integration. Tracked as a follow-up issue. Note that this document's suggested
  implementation pattern cites `internal/eebus/hydraulictemp.go`, which no longer exists
  on `main`; use `internal/eebus/capabilities.go` / `discovery.go` as the current
  reference for a direct feature read.
- The "confirmed absent" table below remains the authoritative answer for this hardware.
  The missing `softwareRevision`/`hardwareRevision` finding was independently
  re-encountered during PR #157 hardware testing — consult this table before probing
  for a value again.
