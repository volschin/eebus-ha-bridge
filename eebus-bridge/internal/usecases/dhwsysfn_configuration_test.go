...
func TestUpstreamDHWSystemFunctionConfigurationSelectsEebusGoCDSF(t *testing.T) {
	facade := NewUpstreamDHWSystemFunctionConfiguration(clientUsecaseLocalEntity(t))
	if _, ok := facade.UseCase().(*cacdsf.CDSF); !ok {
		t.Fatalf("UseCase() = %T, want *cdsf.CDSF", facade.UseCase())
	}
}

func TestUpstreamCDSFFacadeUsesPublicCapabilities(t *testing.T) {
	...
	client.EXPECT().WriteCapabilities(entity).Return(ucapi.DHWSystemFunctionWriteCapabilities{
		OperationMode: true, StartOneTimeDhw: true, StopOneTimeDhw: true,
	}, nil)
	client.EXPECT().OperationModes(entity).Return([]ucapi.HvacOperationModeType{
		ucapi.HvacOperationModeTypeAuto, ucapi.HvacOperationModeTypeOn, ucapi.HvacOperationModeTypeOff,
	}, nil)
	...
	if !state.ModeWritable || !state.BoostWritable ||
		len(state.AvailableModes) != 3 || state.AvailableModes[2] != "off" {
		t.Fatalf("State() = %+v", state)
	}
}

func TestUpstreamCDSFFacadeRequiresBothBoostDirections(t *testing.T) {
	for _, test := range []struct {
	...
	}{
		{
			name: "start and stop",
			capabilities: ucapi.DHWSystemFunctionWriteCapabilities{
				StartOneTimeDhw: true, StopOneTimeDhw: true,
			},
			wantBoost: true,
		},
		{
			name:         "start only",
			capabilities: ucapi.DHWSystemFunctionWriteCapabilities{StartOneTimeDhw: true},
		},
		{
			name:         "stop only",
			capabilities: ucapi.DHWSystemFunctionWriteCapabilities{StopOneTimeDhw: true},
		},
	} {
		t.Run(test.name, func(t *testing.T) {
			entity := spinemocks.NewEntityRemoteInterface(t)
			client := ucmocks.NewCaCDSFInterface(t)
			client.EXPECT().WriteCapabilities(entity).Return(test.capabilities, nil)
			state, err := (upstreamDHWSystemFunctionCapabilityInspector{client: client}).State(entity)
			if err != nil || state.BoostWritable != test.wantBoost {
				t.Fatalf("State() = %+v, %v; want boost=%t", state, err, test.wantBoost)
			}
		})
	}
}

func TestUpstreamCDSFFacadePreservesAmbiguousModesForValidation(t *testing.T) {
	entity := spinemocks.NewEntityRemoteInterface(t)
	client := ucmocks.NewCaCDSFInterface(t)
	...
	client.EXPECT().OperationModes(entity).Return([]ucapi.HvacOperationModeType{
		ucapi.HvacOperationModeTypeOn, ucapi.HvacOperationModeTypeOn,
	}, nil)

	state, err := (upstreamDHWSystemFunctionCapabilityInspector{client: client}).State(entity)
	if err != nil || len(state.AvailableModes) != 2 {
		t.Fatalf("State() = %+v, %v; duplicate relation must remain visible", state, err)
	}
}
...
func TestAwaitDHWWriteRejectsMissingOrMismatchedCounter(t *testing.T) {
	err := awaitDHWWrite(context.Background(), "DHW boost", func(dhwResultCallback) (*model.MsgCounterType, error) {
		return nil, nil
	})
	...
	if err == nil || !strings.Contains(err.Error(), "unexpected message counter") {
		t.Fatalf("mismatched counter error = %v", err)
	}
}
...
func TestCDSFConfigurationFacadeDelegatesPublicContract(t *testing.T) {
	entity := spinemocks.NewEntityRemoteInterface(t)
	...
	if boost.active == nil || !*boost.active || mode.mode != "off" {
		t.Fatalf("writers received boost=%v mode=%q", boost.active, mode.mode)
	}
}
...
func TestCDSFConfigurationFacadeDoesNotFallbackBetweenWriters(t *testing.T) {
	...
	if !errors.Is(err, writeErr) || boost.calls != 1 || mode.calls != 0 {
		t.Fatalf("error=%v calls=boost:%d mode:%d", err, boost.calls, mode.calls)
	}
}
...
func TestCDSFConfigurationFacadeFailsClosedWithoutDependencies(t *testing.T) {
	constructors := map[string]*CDSFConfigurationFacade{
		"nil upstream local entity": NewUpstreamDHWSystemFunctionConfiguration(nil),
		"nil upstream client":       newUpstreamDHWSystemFunctionConfiguration(nil),
		"empty facade":              {},
		"nil facade":                nil,
	}
...
func TestUpstreamDHWCapabilitiesMapUnavailableData(t *testing.T) {
	if _, err := (upstreamDHWSystemFunctionCapabilityInspector{}).State(nil); !errors.Is(err, ErrDHWSysFnDataUnavailable) {
		t.Fatalf("missing client State() error = %v", err)
	}

	entity := spinemocks.NewEntityRemoteInterface(t)
	client := ucmocks.NewCaCDSFInterface(t)
	client.EXPECT().WriteCapabilities(entity).Return(
		ucapi.DHWSystemFunctionWriteCapabilities{}, eebusapi.ErrDataNotAvailable,
	)
	if _, err := (upstreamDHWSystemFunctionCapabilityInspector{client: client}).State(entity); !errors.Is(err, ErrDHWSysFnDataUnavailable) {
		t.Fatalf("upstream State() error = %v, want ErrDHWSysFnDataUnavailable", err)
	}
}
