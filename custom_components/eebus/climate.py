...
class EebusRoomHeatingClimate(EebusEntity, ClimateEntity):
    ...
    def _setpoint(self) -> SetpointState | None:
        ...
        return data.hvac.setpoint if data is not None else None

    def _system_function(self) -> SystemFunctionState | None:
        data = self.coordinator.data
        return data.hvac.system_function if data is not None else None
    ...
    def available(self) -> bool:
        if not super().available:
            return False
        data = self.coordinator.data
        if data is None or data.capabilities.room_heating != CapabilityState.AVAILABLE:
            return False
        system = self._system_function()
        if system is not None:
            mode = system.operation_mode
            if mode and mode not in _EEBUS_TO_HA_MODE:
                _LOGGER.debug("EEBUS room heating mode %r has no HA mapping", mode)
                return False
        return bool(
            (data.measurements.room_temperature_c is not None and is_fresh(data, StateField.ROOM_TEMPERATURE_C))
            or (self._setpoint() is not None and is_fresh(data, StateField.ROOM_HEATING_SETPOINT))
        )
    ...
    def supported_features(self) -> ClimateEntityFeature:
        ...
        system = self._system_function()
        if system is not None and system.mode_writable and system.available_modes:
    ...
    def current_temperature(self) -> float | None:
        data = self.coordinator.data
        value = data.measurements.room_temperature_c if data is not None else None
    ...
    def hvac_modes(self) -> list[HVACMode]:
        ...
        if system is None:
            return [HVACMode.OFF]
        return [_EEBUS_TO_HA_MODE[mode] for mode in system.available_modes if mode in _EEBUS_TO_HA_MODE] or [
            HVACMode.OFF
        ]
    ...
    def hvac_mode(self) -> HVACMode | None:
        ...
        if system is None:
            return None
        return _EEBUS_TO_HA_MODE.get(system.operation_mode)
