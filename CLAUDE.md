...
- Releases are tag-triggered: pushing a `v*` tag builds multi-arch (amd64/arm64/armv7) Docker images to GHCR. `release-drafter` drafts notes on merge to main. **Bumping `version` in `manifest.json` **and** `eebus-bridge-addon/config.yaml` and pushing the `v*` tag is a maintainer release step, done in its own commit after a PR merges — don't bump them as part of a feature/fix PR.** The two versions must match (`scripts/check_addon_version.py`, enforced in CI): the add-on's `version` is what the Supervisor passes as `BUILD_VERSION`, which is the tag of the GHCR bridge image the add-on packages.
- CI skips Go/proto jobs via path filters when only the other side changed. `paths-ignore` skips `**/*.md`.
- `docs/refactoring-optimization-spec-v4.md` (SPEC4-01..11, code-derived) is the current factual basis for structural work. The earlier RF-* / SPEC2-* / SPEC3-* specs were deleted in #149 — their outcomes are in the code and git history, not in a tracking doc; don't cite them as open work. Two narrow SPEC2 slices still have standalone docs: `docs/spec2-07-ohpcf-stream-consume.md` and `docs/spec2-09-lpc-numeric-validation.md`.
- eebus-go upstream migrations get their own spec per use case: `docs/dhw-cdsf-upstream-migration-spec.md` (done) and `docs/room-heating-eebus-go-migration-spec.md` (proposal). Both follow the same shape — phased ownership handover, no per-request fallback between upstream and legacy writer, legacy deletion only after hardware acceptance. `eebus-bridge/UPSTREAM_PATCHES.md` is the authoritative fork patch inventory and carries the known upstream limitations (CDSF `WriteCapabilities` fail-closed ambiguity, missing `NewFeature` sentinels).
