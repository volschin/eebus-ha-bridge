...
- `docs/refactoring-optimization-spec-v2.md` (SPEC2-01..13, code-derived, supersedes RF-* as the factual basis going forward) tracks a separate priority-ordered list starting from semantic correctness. SPEC2-04 (one canonical-SKI normalizer per language, `ski.py`/`eebus.NormalizeSKI`, config-entry migration to version 2), SPEC2-01 (device-level `connected`/`last_transition` split from bridge `GetStatus`, threadsafe registry-backed per-SKI connection tracking; `device_ready` deferred to SPEC2-12), and SPEC2-02 (LPC heartbeat is bridge-lifecycle-scoped — starts/stops exactly once with the bridge, not per HA device entry; the per-device `switch.eebus_heartbeat` is removed, `StartHeartbeat`/`StopHeartbeat` RPCs marked deprecated), SPEC2-06 (Python-only: `CapabilityState` enum — `unknown`/`available`/`temporarily_unavailable`/`unsupported` — replaces the 7 ambiguous `bool | None` support flags in `coordinator.py`/`snapshot.py`/`models.py` via one shared transition rule; the OHPCF select is now created deterministically at platform setup instead of only when already supported in the first snapshot), and SPEC2-08-partial (internal-only: grouped immutable domain dataclasses — `ConnectionState`/`MeasurementsState`/`LPCState`/`DHWState`/`HVACState`/`OHPCFState`/`CapabilitiesState` in new `state.py` — plus a shared pure `StateReducer` (`apply_reading`/`next_capability_state`) that both the poll path and stream/event handlers apply updates through identically, flattened to the unchanged public `CoordinatorSnapshot` shape via `state.flatten`; the `DeviceSession`/`SnapshotPoller`/`StreamSupervisor`/`ProviderManager` split and narrow entity selectors are deferred to later slices) are done; SPEC2-03/05/07..13 (SPEC2-07 partially started) are open.
